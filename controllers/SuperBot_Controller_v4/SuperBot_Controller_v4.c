...
static WbDeviceTag gripper_motors1[3];	//后面的抓手，其函数在原后面跟个1
static WbDeviceTag camera[3];
WbDeviceTag gps;
WbDeviceTag compass;
...
//----message
//格式：gpsvalue[0],gpsvalue[1],state,my_direction,avoid_flag,canpass
double message_send[6];
double message_receive[6];
double gps_values1[2];
double my_direction = 0;
double other_direction = 0;
//double receive_message[4];
double avoid_flag = 0;
double other_avoid_flag = 0;
int other_state = 0;
double waiting_pass = 0;
double youcan_pass = 0;
...
double fixed_posture_travelaround1[12][3] =
{
    {1.05, 0.00, PI},      //右
    {1.05, -1.05, PI},     //右上
    {1.05, -1.05, 3 * PI / 2},     //右上 转
    {0.00, -1.05, 3 * PI / 2},     //上
    {-1.05, -1.05, 3 * PI / 2},    //左上
    {-1.05, -1.05, PI * 2},        //左上 转
    {-1.05, 0, PI * 2},            //左
...
double fixed_posture_findempty1[4][3] =
{
  {1.05, 0.00, PI},          //右
  {0.00, -1.05, 3 * PI / 2}, //上
  {-1.05, 0, 0},             //左
...
double fixed_posture_loadItem1[4][3] =
{
  {1.05, 0.00, 0},          //右
  {0.00, -1.05, PI / 2},    //上
  {-1.05, 0, PI},           //左
...
void lift1(double position);
void moveFingers(double position);
void moveFingers1(double position);
void init_all();
...
void get_compass_angle1(double* ret_angle);
...
bool targetpos_reached1(double target_posture[], double pos_threshold);
int name2index(char* name);
char* index2name(int index);

//----
void send_message(int main_Sate);
void receive_message();
bool IsAvoid(int my_mainstate, int other_mainstate);
bool IsSameDirection();
void wait();
void waiting();
double caculate_distance(double gps_values[], double gps_values1[]);
void avoid();
...
bool Aim_and_Grasp1(int* grasp_state, WbDeviceTag camera, int objectID);
...
int main(int argc, char** argv)
{
    ...
    {
        step();
		send_message(main_state);
		receive_message();
		//printf("4:%f", avoid_flag);
		printf("4:%.1f\n", waiting_pass);
		if (IsAvoid(main_state, other_state))
		{
			//Robot_State_Machine(&Avoid, &grasp_state);
			avoid();

		}
		else if (waiting_pass == 1)
		{
			waiting();
...
void init_all()
{
    ...
    camera[0] = wb_robot_get_device("camera_back_up"); //相机初始化
    camera[1] = wb_robot_get_device("camera_front_down");
    camera[2] = wb_robot_get_device("camera_back_down");
	...
	//----emitter 和 receiver初始化
	emitter = wb_robot_get_device(emitterd);
	receiver = wb_robot_get_device(receiverd);
    ...
    gripper_motors[0] = wb_robot_get_device("front_lift motor");
    gripper_motors[1] = wb_robot_get_device("front_right_finger motor");
    gripper_motors[2] = wb_robot_get_device("front_left_finger motor");
    gripper_motors1[0] = wb_robot_get_device("back_lift motor");
    gripper_motors1[1] = wb_robot_get_device("back_right_finger motor");
    gripper_motors1[2] = wb_robot_get_device("back_left_finger motor");

    //电机加力反馈
    wb_motor_enable_force_feedback(gripper_motors[1], 1);
    wb_motor_enable_force_feedback(gripper_motors[2], 1);
    wb_motor_enable_force_feedback(gripper_motors1[1], 1);
    wb_motor_enable_force_feedback(gripper_motors1[2], 1);
...
void Robot_State_Machine(int* main_state, int* grasp_state)
{
    ...
    case Recognize_Empty:
    {
        if (Find_Empty(camera[0])) //货架上有空位
        {
            ...
            {
                NextShelf = CurrentShelf + 1;//换到下一货架,
                NextShelf %= 4;
        ...
        else //货架上无空位
        {
            if (!goodcount) {
                ...
                *main_state = RunTo_NextShelf;//前往下一个货架
                printf("main_state changes from Recognize_Empty to RunTo_NextShelf!\n");               
                if (oddcount >= 6)*main_state = Arround_Moving;
                oddcount++;
            }
        }
        break;
    ...
    case Arround_Moving:
    {
        ...
        if (goodcount) {
            ...
            if (Find_Goods(camera[2], index2name(TargetGood), &Item_Grasped_Id))
            {
                *main_state = Grab_Item;
                printf("main_state changes from Arround_Moving to Grab_Item!\n");
                set_posture(fin_target_posture, gps_values[0], gps_values[1], compass_angle);
            ...
            else
            {
                ...
                {
                    travel_points_sum++;
                    printf("GoodsonShelf[%d][%d] need %s\n", NextShelf, TargetIndex, index2name(TargetGood));
                    set_posture(initial_posture, gps_values[0], gps_values[1], compass_angle);
                    set_posture(fin_target_posture, fixed_posture_travelaround1[Travel_Point_Index][0], fixed_posture_travelaround1[Travel_Point_Index][1], fixed_posture_travelaround1[Travel_Point_Index][2]);
                    Travel_Point_Index += 1;
                    Travel_Point_Index %= 12;
                }
            }
    ...
    case Grab_Item:
    {
        if (goodcount) {
            if (Aim_and_Grasp(grasp_state, camera[1], Item_Grasped_Id)) {
                goodcount--;
                if (oddcount >= 6)*main_state = Back_Moving;
                else {
                    myswap(&NextGood, &TargetGood);
                    myswap(&NextIndex, &TargetIndex);
                    myswap(&CurrentShelf, &NextShelf);
                    *main_state = Arround_Moving;
                    *grasp_state = 0;
                    flag = 0;
                }
            ...
            if (Aim_and_Grasp1(grasp_state, camera[2], Item_Grasped_Id)) {
                printf("抓到回去啦！\n");
            ...
            if (Moveto_CertainPoint(fin_target_posture, 0.05))
            {
                if (fin_target_posture[0] == fixed_posture_findempty[CurrentShelf][0] && fin_target_posture[1] == fixed_posture_findempty[CurrentShelf][1])
                {
                    *main_state = TurnBack_To_LoadItem;
                    ...
                    set_posture(initial_posture, gps_values[0], gps_values[1], compass_angle);
                    //设置下一目标点为反向180度
                    set_posture(fin_target_posture, fixed_posture_loadItem1[CurrentShelf][0], fixed_posture_loadItem1[CurrentShelf][1], fixed_posture_loadItem1[CurrentShelf][2]);
                    printf("到上货的地方了！\n");
                    travel_points_sum = 0;
                }
                else
    ...
    case Item_Loading:
    {
        if (!goodcount) {
            ...
            if (Moveto_CertainPoint(load_target_posture, 0.001))
            {
                ...
                goodcount++;
                lift(height = 0.020);
                if(oddcount>=6)*main_state = Init_Pose;
                else {
                    *main_state = Back_Moving;
                    myswap(&NextGood, &TargetGood);
                    myswap(&NextIndex, &TargetIndex);
    ...
    case RunTo_NextShelf:
    {
        if (Moveto_CertainPoint(fin_target_posture, 0.05))
        {
            ...
            if (!goodcount && fin_target_posture[0] == fixed_posture_findempty[CurrentShelf][0] && fin_target_posture[1] == fixed_posture_findempty[CurrentShelf][1])
            {
                printf("到下一个货架%d了！\n", CurrentShelf);
                *main_state = Recognize_Empty;
            ...
            else if (goodcount && fin_target_posture[0] == fixed_posture_findempty[NextShelf][0] && fin_target_posture[1] == fixed_posture_findempty[NextShelf][1])
            {
                printf("到下一个货架%d了！\n", NextShelf);
                *main_state = Recognize_Empty;
                printf("main_state changes from RunTo_NextShelf to Recognize_Empty!\n");
...
bool Moveto_CertainPoint1(double fin_posture[], double reach_precision)
{
    if (targetdist_reached(fin_posture, reach_precision) && targetpos_reached1(fin_posture, reach_precision))
    {
        // printf("到达目标位置！\n");
        // base_reset();
        return true;
        ...
        if (objects[i].id == objectID) //找到画面中第一个ID物体
        {
            if (*grasp_state == 0) //调整位置
            {
                ...
                //水瓶特别提高一点
                    ...
                    lift1(height = 0.10);
                else lift1(height = 0.0);
                moveFingers1(width = objects[i].size[0] / 1.5);
                // printf("ID %d 的物体 %s 在 %lf %lf\n", objects[i].id, objects[i].model, objects[i].position[0], objects[i].position[2]);
                get_gps_values(gps_values);
                get_compass_angle(&compass_angle);
                double grasp_target_posture[3];
                ...
                if (fabs(objects[i].position[0]) < grasp_threshold && fabs(objects[i].position[2] - grasp_dis_set) < grasp_threshold)
                {
                    *grasp_state += 1;
                    ...
                    // 用视觉先来个抓手基本值
                    printf("物体大小: %lf %lf\n", objects[i].size[0], objects[i].size[1]);
                    moveFingers1(width = objects[i].size[0] / 2);
                    wb_robot_step(30000 / TIME_STEP);
                }
            }
            else if (*grasp_state == 1) //抓
            {
                double grasp_force_threshold = 50.0;
                if (wb_motor_get_force_feedback(gripper_motors1[1]) > -grasp_force_threshold)
                    moveFingers1(width -= 0.0003); //步进
                else
                {
                    printf("当前电机力反馈：%.3f\n", wb_motor_get_force_feedback(gripper_motors1[1]));
                    printf("抓紧了！\n");
                    wb_robot_step(30000 / TIME_STEP); //等他抓稳定
                    if (wb_motor_get_force_feedback(gripper_motors1[1]) <= -grasp_force_threshold)
                    {
    ...
    if (!goodcount) {
        ...
        for (int j = 0; j < 16; j++)
        {
            ...
            if (GoodsonShelf[CurrentShelf][j] == -1)
            {
                ...
                if (j % 8 < 4)
                    for (int k = 0; k < 8; k++)//从左往右
                    {
                        if (GoodsonShelf[CurrentShelf][TargetFloor + k] != -1)
                        {
                            ...
                            if (!goodcount) TargetGood = GoodsonShelf[CurrentShelf][TargetFloor + k];
                            else NextGood = GoodsonShelf[CurrentShelf][TargetFloor + k];
                            break;
                        }
                    }
                else
                ...
                //如果整排都没有可能会出错 下次一定
                if (!goodcount) printf("First GoodsonShelf[%d][%d] need %s\n", CurrentShelf, j, index2name(TargetGood));
                else printf("Next GoodsonShelf[%d][%d] need %s\n", CurrentShelf, j, index2name(NextGood));
                break;
            }
        }
        if (Empty_Flag)
        ...
        // printf("识别到 %d 个物体.\n", number_of_objects);
        const WbCameraRecognitionObject* objects = wb_camera_recognition_get_objects(camera);
        int GoodsonShelf1[4][16];      //货架上的物品ID号 先下后上 先左后右
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 16; j++)
                GoodsonShelf1[i][j] = -1;//不知道为啥写在定义的时候不成功
        for (int i = 0; i < number_of_objects; ++i)
        {
            // printf("物体 %d 的类型: %s ", i, objects[i].model);
            // printf("ID: %d ", objects[i].id);
            ...
            GoodsonShelf1[NextShelf][Shelfy * 8 + Shelfx] = name2index(objects[i].model);
            //printf("%d  %d\n",GoodsonShelf1[NextShelf][Shelfy * 8 + Shelfx], Shelfy * 8 + Shelfx);
            printf("物体 %s 对应编号 %d 写入[%d] 写入编号为%d\n", objects[i].model, name2index(objects[i].model), Shelfy * 8 + Shelfx, GoodsonShelf1[NextShelf][Shelfy * 8 + Shelfx]);
        ...
        for (int j = 0; j < 16; j++)
            printf("GoodsonShelf[%d][%d] = %d\n", NextShelf, j, GoodsonShelf1[NextShelf][j]);
        for (int j = 0; j < 16; j++)
        {
            // if (strlen(GoodsonShelf[CurrentShelf][j]) == 0) // 这种判断可能会crash
            //   Empty_Flag = 1;
            if (GoodsonShelf1[NextShelf][j] == -1)
            {
                Empty_Flag = 1;
                if (!goodcount)TargetIndex = j;
                else NextIndex = j;
                ...
                if (j % 8 < 4)
                    for (int k = 0; k < 8; k++)//从左往右
                    {
                        if (GoodsonShelf1[NextShelf][TargetFloor + k] != -1)
                        {
                            // strcpy(TargetGood, GoodsonShelf[CurrentShelf][TargetFloor + k]);
                            if (!goodcount) TargetGood = GoodsonShelf1[NextShelf][TargetFloor + k];
                            else NextGood = GoodsonShelf1[NextShelf][TargetFloor + k];
                            break;
                        }
                    }
                else
                ...
                //如果整排都没有可能会出错 下次一定
                if (!goodcount) printf("First GoodsonShelf[%d][%d] need %s\n", NextShelf, j, index2name(TargetGood));
                else printf("Next GoodsonShelf[%d][%d] need %s\n", NextShelf, j, index2name(NextGood));
                break;
            }
        }
        if (Empty_Flag)
...
void lift1(double position)
{
    wb_motor_set_velocity(gripper_motors1[0], GRIPPER_MOTOR_MAX_SPEED);
    wb_motor_set_position(gripper_motors1[0], position);
}
...
void moveFingers1(double position)
{
    wb_motor_set_velocity(gripper_motors1[1], GRIPPER_MOTOR_MAX_SPEED);
    wb_motor_set_velocity(gripper_motors1[2], GRIPPER_MOTOR_MAX_SPEED);
    wb_motor_set_position(gripper_motors1[1], position);
    wb_motor_set_position(gripper_motors1[2], position);
}
...
bool targetpos_reached1(double target_posture[], double pos_threshold)
{
    get_compass_angle1(&compass_angle);
    double angle = target_posture[2] - compass_angle;
    if (fabs(angle) <= pos_threshold || fabs(angle) >= 2 * PI - pos_threshold)
        return true;
    return false;
...
void get_compass_angle1(double* ret_angle)
{
    ...
    const double v_front[2] = { compass_raw_values[0], compass_raw_values[1] };
    const double v_north[2] = { 1.0, 0.0 };
    *ret_angle = vector2_angle(v_front, v_north); // angle E(0, 2*PI)
    // printf("当前姿态：%.3f  rad\n", *ret_angle);
}
...
{
	get_gps_values(gps_values);
	message_send[0] = gps_values[0];
	message_send[1] = gps_values[1];
	message_send[2] = main_Sate;
	message_send[3] = my_direction;
	message_send[4] = avoid_flag;
	message_send[5] = youcan_pass;
	int index = 0;
	for (index = 0; index < 6; index++)
	{
		wb_emitter_send(emitter, &message_send[index], sizeof(double));
	}
	//printf("4已发送");
	//printf("4:x:%.3f, y:%.3f", gps_values[0], gps_values[1]);
...
void receive_message()
{
	...
	while (wb_receiver_get_queue_length(receiver) > 0) {
		...
		if (index < 6)
		{
			message_receive[index] = *p;
			index++;
		}
		wb_receiver_next_packet(receiver);
	}
	gps_values1[0] = message_receive[0];
	gps_values1[1] = message_receive[1];
	other_state = (int)message_receive[2];
	other_avoid_flag = message_receive[4];
	other_direction = message_receive[3];
	if (message_receive[5] == 1)
	{
		waiting_pass = 0;
	}
	//printf("4已接收");
...
double caculate_distance(double gps_values[], double gps_values1[])
{
	return ((gps_values[0] - gps_values1[0])*(gps_values[0] - gps_values1[0]) + (gps_values[1] - gps_values1[1])*(gps_values[1] - gps_values1[1]));
}
...
bool MeClose(double x, double y)
{
	double MyDis = (gps_values[0] - x)*(gps_values[0] - x) + (gps_values[1] - y)*(gps_values[1] - y);
	double OthDis = (gps_values1[0] - x)*(gps_values1[0] - x) + (gps_values1[1] - y)*(gps_values1[1] - y);
	if (MyDis >= OthDis)
		return false;
	else
		return true;
...
bool MeSideClose(double x, double y, double x1, double y1)
{
	double MyDis = (gps_values[0] - x)*(gps_values[0] - x) + (gps_values[1] - y)*(gps_values[1] - y);
	double OthDis = (gps_values1[0] - x1)*(gps_values1[0] - x1) + (gps_values1[1] - y1)*(gps_values1[1] - y1);
	if (MyDis > OthDis)
		return false;
	else
		return true;
...
bool IsAvoid(int my_main_state, int other_mainstate)
{
	get_gps_values(gps_values);
	gps_values1[0] = message_receive[0];
	gps_values1[1] = message_receive[1];
	double distance = caculate_distance(gps_values, gps_values1);
	printf("4distance: %.2f\n", distance);
	if (IsSameDirection())//正向,同向
	{
		printf("同向\n");
		if (fabs(gps_values[0]) > 0 && fabs(gps_values[1]) >0) {
			if (my_direction == 0)//逆时针
			{
				if (gps_values[0] < -0.6)
				{
					if (gps_values[1] > 0)
					{
						if (gps_values1[1] > 0.6)
						{
							//avoid_flag = 1;
							set_posture(temp_avoid_posture, -1.05, 0, compass_angle);
							return true;
						}
						else
						{
				...
				else if(gps_values[0]>0.6 && gps_values[1]<0)
				{
					if (gps_values1[1] < -0.6)
					{
						//avoid_flag = 1;
				...
				else if (gps_values[1] > 0.6 && gps_values[0] > 0)
				{

					if (gps_values1[0] > 0.6)
					{
						//avoid_flag = 1;
						set_posture(temp_avoid_posture, 0, 1.05, compass_angle);
						return true;
					}
					else
						return false;
	...
	else//异向
	{
		...
		if (avoid_flag == 1)
			return true;
		else if (other_avoid_flag == 1)
			return false;
		else if (distance < 2)
		{
			if (my_main_state == Grab_Item || my_main_state == TurnBack_To_LoadItem || my_main_state == Item_Loading)
			{
				printf("special\n");
				avoid_flag = 0;
				return false;
			}
			else if (other_mainstate == Grab_Item || other_mainstate == TurnBack_To_LoadItem || other_mainstate == Item_Loading)
			{
				printf("specical\n");
				avoid_flag = 1;
				return true;
			...
			if (my_direction == 0)
			{
				if (fabs(gps_values[0]) > fabs(gps_values[1]))
				{
					if (fabs(gps_values1[1]) > fabs(gps_values1[0]))
					{
						if (gps_values[0] > 0 && gps_values1[1] < 0 )
						{
							if (MeClose(1.7, -1.7))
							{
								set_posture(initial_posture, gps_values[0], gps_values[1], compass_angle);
								set_posture(temp_avoid_posture, 1.7, -1.7, compass_angle);
								avoid_flag = 1;
								youcan_pass = 0;
								printf("turn");
								return true;
						...
						else if(gps_values[0] < 0 && gps_values1[1] > 0 )
						{
					...
					else if(fabs(gps_values1[0]) > fabs(gps_values1[1]))
					{
						if (gps_values[0] > 0 && gps_values1[0] > 0 && gps_values[1]>gps_values1[1])//在一边且逆向
						{
							if (MeSideClose(1.7, 1.7, 1.7, -1.7))
							{
								set_posture(initial_posture, gps_values[0], gps_values[1], compass_angle);
								set_posture(temp_avoid_posture, 1.7, 1.7, compass_angle);
								avoid_flag = 1;
						...
						else if (gps_values[0] < 0 && gps_values1[0] < 0 && gps_values[1]<gps_values1[1])
						{
							if (MeSideClose(-1.7, -1.7, -1.7, 1.7))
							{
								set_posture(initial_posture, gps_values[0], gps_values[1], compass_angle);
				...
				else
				{
					if (fabs(gps_values1[1]) < fabs(gps_values1[0]))
					{
						if (gps_values[1] < 0 && gps_values1[0] < 0  )
						{
							if (MeClose(-1.7, -1.7))
							{
								set_posture(initial_posture, gps_values[0], gps_values[1], compass_angle);
						...
						else if (gps_values[1] > 0 && gps_values1[0] > 0  )
						{
							if (MeClose(1.7, 1.7))
							{
								set_posture(initial_posture, gps_values[0], gps_values[1], compass_angle);
					...
					{
						if (gps_values[1] > 0 && gps_values1[1] > 0 && gps_values[0] < gps_values1[0])//在一边且逆向
						{
							if (MeSideClose(-1.7, 1.7, 1.7, 1.7))
							{
								set_posture(initial_posture, gps_values[0], gps_values[1], compass_angle);
						...
						else if (gps_values[1] < 0 && gps_values1[1] < 0 && gps_values[0] > gps_values1[0])
						{
							if (MeSideClose(1.7, -1.7, -1.7, -1.7))
							{
								set_posture(initial_posture, gps_values[0], gps_values[1], compass_angle);
			...
			else
			{
				if (fabs(gps_values[0]) > fabs(gps_values[1]))
				{
					...
					{
						if (gps_values[0] > 0 && gps_values1[0] > 0 && gps_values[1] < gps_values1[1])
						{
							if (MeSideClose(1.7, -1.7, 1.7, 1.7))
							{
								set_posture(initial_posture, gps_values[0], gps_values[1], compass_angle);
...
void back()
{
	temp_avoid_posture[0] = (temp_avoid_posture[0] / fabs(temp_avoid_posture[0]))*1.05;
	temp_avoid_posture[1] = (temp_avoid_posture[1] / fabs(temp_avoid_posture[1]))*1.05;
	if (Moveto_CertainPoint(temp_avoid_posture, 0.05))
	{
		printf("4move to %.3f %.3f", temp_avoid_posture[0], temp_avoid_posture[1]);
		avoid_flag = 0;
	}
}
...
void wait()
{
	...
	{
		temp_avoid_posture[0] = (temp_avoid_posture[0] / fabs(temp_avoid_posture[0]))*1.7;
		temp_avoid_posture[1] = (temp_avoid_posture[1] / fabs(temp_avoid_posture[1]))*1.7;
		avoid_flag = 1;
	}
	return;
}
...
void waiting()
{
	set_posture(temp_avoid_posture, gps_values[0], gps_values[1], compass_angle);
	Moveto_CertainPoint(temp_avoid_posture, 0.05);
}
...
void avoid()
{
	if (IsSameDirection())
	{
		//set_posture(temp_avoid_posture, gps_values[0], gps_values[1], compass_angle);
		Moveto_CertainPoint(temp_avoid_posture, 0.05);
		printf("4In front of me , there is a car, I'm wait....\n");
	}
	else
	{
		if (Moveto_CertainPoint(temp_avoid_posture, 0.05))
